...
def test_register_and_get_config():
    user_data = {"email": "testuser3@example.com", "password": "mysecurepassword"}
...
def test_evaluate_endpoint(tmp_path):
    # This assumes you have a test GT file and predictions ZIP available locally
    # and that your API expects 'config', 'ground_truth', 'predictions'
    user_data = {"email": "evaluser@example.com", "password": "evaluategood"}
    resp = client.post("/auth/register", json=user_data)
    assert resp.status_code == 201
    resp = client.post("/auth/jwt/login", data={"username": user_data["email"], "password": user_data["password"]})
    assert resp.status_code == 204
    cookies = resp.cookies.get("fastapiusersauth")
    ...
    data = {
        "config": json.dumps(config) # or json.dumps(config) depending on your API
    }
    with open(gt_path, "rb") as gt_file, open(predictions_path, "rb") as pred_file:
        files = {
            "ground_truth": ("gt_test.xlsx", gt_file, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
    ...
    assert resp_eval.headers['content-type'].startswith("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
